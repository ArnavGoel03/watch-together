...
- **Safari.** `safari/` holds an Xcode project, generated by Apple's own converter, whose
  every path REFERENCES `extension/` rather than copying it, so one extension directory
  now feeds Chrome, Firefox and Safari and a content script change reaches all three with
  no copy step to forget. Both targets build: macOS and iOS. Two things the converter got
  wrong are fixed and guarded: it dragged `manifest.firefox.json` and
  `background-firefox.js` into an Apple binary, and it seeded the app version at 1.0
  while the extension said 1.2.2, which `scripts/check-version.mjs` now fails the build
  over. Nothing here is submitted: Safari distribution needs the Apple Developer Program
  at ninety nine dollars a year. See `docs/SAFARI.md`.
